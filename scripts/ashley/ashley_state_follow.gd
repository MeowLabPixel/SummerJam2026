...
func physics_update(delta: float) -> void:
	...
	# Hysteresis: rest state only changes on threshold crossings.
	if _is_resting:
	...
	# Build repulsion vector from all nearby threats.
	...
	# Resting: full stop, no repulsion nudges.
	# Accumulate cornered timer while a zombie is actively pushing.
	if _is_resting:
